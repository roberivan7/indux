...
try {
    ...
    foreach ($rows as $r) {
        $stats[$r['status']] = (int)$r['qtd'];
        $stats['total'] += (int)$r['qtd'];
    }

    // Alarmes
    $r = $db->query("SELECT COUNT(*) FROM alarmes WHERE resolvido = 0");
    $alarmesTotal = (int)$r->fetchColumn();
    $r = $db->query("SELECT COUNT(*) FROM alarmes WHERE resolvido = 0 AND severidade = 'critico'");
    $alarmesCriticos = (int)$r->fetchColumn();
    ...
    $stats = ['total' => 8, 'ativos' => 5, 'inativos' => 2, 'em_falha' => 1];
    $alarmesCriticos = 2;
    $alarmesTotal    = 4;
}
?>
    ...
    <div class="kpi-grid">
      <div class="kpi-card kpi-card--cyan">
        ...
        <div>
          ...
          <div class="kpi-valor"><?php echo $stats['total']; ?></div>
        </div>
      </div>
      <div class="kpi-card kpi-card--green">
        ...
        <div>
          ...
          <div class="kpi-valor"><?php echo $stats['ativos']; ?></div>
        </div>
      </div>
      <div class="kpi-card kpi-card--blue">
        ...
        <div>
          ...
          <div class="kpi-valor"><?php echo $stats['inativos']; ?></div>
        </div>
      </div>
      <div class="kpi-card <?php echo $stats['em_falha'] > 0 ? 'kpi-card--red' : 'kpi-card--purple'; ?>">
        <div class="kpi-icon">🔴</div>
        <div>
          <div class="kpi-label">Em Falha</div>
          <div class="kpi-valor"><?php echo $stats['em_falha']; ?></div>
        </div>
      </div>
    ...
    <div class="monitor-grid">
          ...
          <table class="tabela-estoque">
            ...
            <tbody>
              <?php foreach ($ultimasLeituras as $l):
                $tClass = avaliarTemp($l['temperatura'], 0, $l['temp_max'] ?: 80);
                $pClass = avaliarPressao($l['pressao'], 0, $l['pressao_max'] ?: 10);
              ?>
              <tr>
                <td>
                  <div style="font-weight:600;font-size:.85rem;color:#fff"><?php echo htmlspecialchars($l['nome']); ?></div>
                  <span class="tag-chip"><?php echo htmlspecialchars($l['tag']); ?></span>
                </td>
                <td class="text-mono leitura-val <?php echo $tClass; ?>"><?php echo number_format($l['temperatura'],1); ?>°</td>
                <td class="text-mono leitura-val <?php echo $pClass; ?>"><?php echo number_format($l['pressao'],2); ?> bar</td>
                <td><span class="status-badge <?php echo statusClass($l['status']); ?>"><?php echo statusLabel($l['status']); ?></span></td>
                <td style="font-size:.72rem;color:var(--text-muted);font-family:var(--font-mono)"><?php echo date('H:i', strtotime($l['registrado_em'])); ?></td>
              </tr>
              <?php endforeach; ?>
            </tbody>
          </table>
      ...
      <div class="panel-card">
        ...
        <div class="panel-body">
          ...
          <?php else: ?>
          <?php foreach ($ultimosAlarmes as $a): ?>
          <div class="alarme-item <?php echo $a['severidade']; ?>">
            <div class="alarme-icon"><?php echo tipoAlarmeIcon($a['tipo']); ?></div>
            <div class="alarme-body">
              <div class="alarme-msg"><?php echo htmlspecialchars($a['mensagem']); ?></div>
              <div class="alarme-meta">
                <span class="tag-chip"><?php echo htmlspecialchars($a['equip_tag']); ?></span>
                <span><?php echo date('d/m H:i', strtotime($a['criado_em'])); ?></span>
              </div>
            </div>
            <span class="severidade-badge sev--<?php echo $a['severidade']; ?>">
              <?php echo severidadeIcon($a['severidade']); ?> <?php echo ucfirst($a['severidade']); ?>
            </span>
          </div>
          <?php endforeach; ?>
          <?php endif; ?>
    ...
    <div class="panel-card" style="margin-top:1.25rem">
      ...
      <div class="panel-body" style="padding:0">
        <table class="tabela-estoque">
          ...
          <tbody>
            <?php foreach ($ultimosEquip as $e): ?>
            <tr>
              <td><span class="tag-chip"><?php echo htmlspecialchars($e['tag']); ?></span></td>
              <td>
                <strong style="color:#fff"><?php echo htmlspecialchars($e['nome']); ?></strong>
                <?php if ($e['modelo']): ?>
                <div style="font-size:.72rem;color:var(--text-muted);font-family:var(--font-mono)"><?php echo htmlspecialchars($e['modelo']); ?></div>
                <?php endif; ?>
              </td>
              <td style="font-size:.82rem;color:var(--text-dim)"><?php echo htmlspecialchars($e['localizacao'] ?: '—'); ?></td>
              <td><span class="status-badge <?php echo statusClass($e['status']); ?>"><?php echo statusLabel($e['status']); ?></span></td>
              <td style="font-size:.75rem;color:var(--text-muted);font-family:var(--font-mono)"><?php echo date('d/m/Y', strtotime($e['criado_em'])); ?></td>
              <td>
                <a href="equipamentos.php?ver=<?php echo $e['id']; ?>" class="btn btn--ghost btn--sm">Ver</a>
              </td>
            </tr>
            <?php endforeach; ?>
          </tbody>
