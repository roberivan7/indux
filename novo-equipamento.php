...
if (isset($_GET['editar'])) {
    ...
    $editando = true;
    try {
        $stmt = getDB()->prepare('SELECT * FROM equipamentos WHERE id = ?');
        $stmt->execute([$editId]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validações
    if ($data['tag'] === '')  $errors[] = 'TAG é obrigatória.';
    if ($data['nome'] === '') $errors[] = 'Nome é obrigatório.';
    if (!in_array($data['status'], ['ativo','inativo','em_falha'])) $errors[] = 'Status inválido.';
    if ($data['temp_max'] <= $data['temp_min']) $errors[] = 'Temperatura máxima deve ser maior que a mínima.';
    if ($data['pressao_max'] <= $data['pressao_min']) $errors[] = 'Pressão máxima deve ser maior que a mínima.';

    if (empty($errors)) {
        try {
            $db = getDB();
            $editId = (int)($_POST['editar_id'] ?? 0);

            if ($editId) {
                // Verificar TAG duplicada (exceto o próprio)
                $chk = $db->prepare('SELECT id FROM equipamentos WHERE tag = ? AND id != ?');
                $chk->execute([$data['tag'], $editId]);
                if ($chk->fetch()) { $errors[] = 'TAG já cadastrada para outro equipamento.'; }
                else {
                    $db->prepare(
                    ...
                    registrarLog('EDITAR_EQUIPAMENTO','equipamentos',$editId,'TAG:'.$data['tag']);
                    header('Location: equipamentos.php?msg='.urlencode('✅ Equipamento atualizado!').'&tipo=sucesso');
                    exit;
                }
            } else {
                $chk = $db->prepare('SELECT id FROM equipamentos WHERE tag = ?');
                $chk->execute([$data['tag']]);
                if ($chk->fetch()) { $errors[] = 'TAG já cadastrada. Use outra identificação.'; }
                else {
                    $db->prepare(
                    ...
                    registrarLog('CRIAR_EQUIPAMENTO','equipamentos',$novoId,'TAG:'.$data['tag']);
                    header('Location: equipamentos.php?msg='.urlencode('✅ Equipamento "'.$data['nome'].'" cadastrado!').'&tipo=sucesso');
                    exit;
                }
            }
        } catch (Throwable $e) {
    ...
    $equipamento = $data;
    $editId = (int)($_POST['editar_id'] ?? 0);
    if ($editId) { $equipamento['id'] = $editId; $editando = true; }
}

$d = $equipamento ?? [];
$editId = $editando && isset($d['id']) ? $d['id'] : 0;
?>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
...
<body>
  ...
  <main class="site-main">
    ...
    <form method="POST" action="novo-equipamento.php">
      <?php if ($editando): ?>
      <input type="hidden" name="editar_id" value="<?php echo $editId; ?>">
      <?php endif; ?>
        ...
        <div class="form-grid">
          <div class="form-group">
            ...
            <input type="text" id="tag" name="tag" class="form-control"
              ...
              value="<?php echo htmlspecialchars($d['tag'] ?? ''); ?>"
          ...
          <div class="form-group">
            ...
            <input type="text" id="nome" name="nome" class="form-control"
              ...
              value="<?php echo htmlspecialchars($d['nome'] ?? ''); ?>"
          ...
          <div class="form-group">
            ...
            <input type="text" id="modelo" name="modelo" class="form-control"
              ...
              value="<?php echo htmlspecialchars($d['modelo'] ?? ''); ?>" maxlength="100">
          </div>
          ...
          <div class="form-group">
            ...
            <input type="text" id="fabricante" name="fabricante" class="form-control"
              ...
              value="<?php echo htmlspecialchars($d['fabricante'] ?? ''); ?>" maxlength="100">
          </div>
          ...
          <div class="form-group">
            ...
            <input type="text" id="localizacao" name="localizacao" class="form-control"
              ...
              value="<?php echo htmlspecialchars($d['localizacao'] ?? ''); ?>" maxlength="200">
          </div>

          <div class="form-group">
            <label class="form-label" for="status">Status Inicial</label>
            <select id="status" name="status" class="form-control">
              <option value="ativo"    <?php echo ($d['status']??'ativo')==='ativo'    ? 'selected' : ''; ?>>✅ Ativo</option>
              <option value="inativo"  <?php echo ($d['status']??'')==='inativo'       ? 'selected' : ''; ?>>⏸️ Inativo</option>
              <option value="em_falha" <?php echo ($d['status']??'')==='em_falha'      ? 'selected' : ''; ?>>🔴 Em Falha</option>
            </select>
          </div>
          ...
          <div class="form-group full">
            ...
            <textarea id="descricao" name="descricao" class="form-control" rows="3"
              placeholder="Descrição técnica, observações, procedimentos especiais..."><?php echo htmlspecialchars($d['descricao'] ?? ''); ?></textarea>
          </div>
        </div>
          ...
          <div class="form-grid">
            <div class="form-group">
              ...
              <input type="number" id="pressao_min" name="pressao_min" class="form-control"
                ...
                value="<?php echo $d['pressao_min'] ?? 0; ?>">
            </div>
            <div class="form-group">
              ...
              <input type="number" id="pressao_max" name="pressao_max" class="form-control"
                step="0.01" min="0" max="1000"
                value="<?php echo $d['pressao_max'] ?? 10; ?>">
  ...
  document.getElementById('tag').addEventListener('input', function() {
    let v = this.value.toUpperCase().replace(/[^A-Z0-9\-_]/g,'');
    this.value = v;
  });
  </script>
