...
if (isset($_GET['resolver']) && podeResolverAlarme()) {
    ...
    try {
        ...
        registrarLog('RESOLVER_ALARME','alarmes',$id);
...
try {
    ...
    foreach ($rows as $r) { $contagens[$r['severidade']] = (int)$r['qtd']; $contagens['total'] += (int)$r['qtd']; }
    $r2 = $db->query("SELECT COUNT(*) FROM alarmes WHERE resolvido=1")->fetchColumn();
    $contagens['resolvidos'] = (int)$r2;
    ...
    if ($filtroResolvido !== '') {
        $alarmes = array_filter($alarmes, fn($a) => (int)$a['resolvido'] === (int)$filtroResolvido);
    }
    if ($filtroSev !== '') {
        $alarmes = array_filter($alarmes, fn($a) => $a['severidade'] === $filtroSev);
    }
}
?>
<!DOCTYPE html>
...
<head>
  ...
  <title>Alarmes — <?php echo SISTEMA_NOME; ?></title>
...
<body>
  ...
  <main class="site-main">
    ...
    <!-- Tabs -->
    <div class="tabs">
      <a href="alarmes.php?res=0" class="tab-btn <?php echo $filtroResolvido==='0'&&$filtroSev==='' ? 'ativo' : ''; ?>">
        🔔 Pendentes <span class="tab-badge tab-badge--red"><?php echo $contagens['total']; ?></span>
      </a>
      <a href="alarmes.php?res=0&sev=critico" class="tab-btn <?php echo $filtroSev==='critico' ? 'ativo' : ''; ?>">
        🔴 Críticos <span class="tab-badge tab-badge--red"><?php echo $contagens['critico']; ?></span>
      </a>
      <a href="alarmes.php?res=0&sev=alerta" class="tab-btn <?php echo $filtroSev==='alerta' ? 'ativo' : ''; ?>">
    ...
    <div>
      ...
      <div class="alarme-item <?php echo $a['severidade']; ?> <?php echo $a['resolvido'] ? 'resolvido' : ''; ?>">
        ...
        <div class="alarme-body">
          <div style="display:flex;align-items:center;gap:.6rem;flex-wrap:wrap;margin-bottom:.3rem">
            ...
            <span style="font-size:.78rem;font-weight:600;color:var(--text-dim)"><?php echo htmlspecialchars($a['equip_nome']); ?></span>
          </div>
          <div class="alarme-msg"><?php echo htmlspecialchars($a['mensagem']); ?></div>
          <div class="alarme-meta" style="margin-top:.3rem">
            <?php if ($a['valor_registrado'] !== null): ?>
            <span>📊 Valor: <strong><?php echo number_format($a['valor_registrado'],2); ?></strong></span>
            <?php endif; ?>
            <?php if ($a['valor_limite'] !== null): ?>
            <span>🚧 Limite: <strong><?php echo number_format($a['valor_limite'],2); ?></strong></span>
            <?php endif; ?>
            <span>🕐 <?php echo date('d/m/Y H:i', strtotime($a['criado_em'])); ?></span>
            <?php if ($a['resolvido'] && $a['resolvido_nome']): ?>
            <span style="color:var(--green)">✅ Resolvido por <?php echo htmlspecialchars($a['resolvido_nome']); ?></span>
            <?php endif; ?>
          </div>
        </div>

        <?php if (!$a['resolvido'] && ehOperador()): ?>
        <a href="alarmes.php?resolver=<?php echo $a['id']; ?>&res=0"
